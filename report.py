...
# ── Magazine Training Excel Raporu ───────────────────────────────────────────

def generate_magazine_report(csv_path: str) -> str:
    from openpyxl import Workbook
    ...
    with open(csv_path, newline="", encoding="utf-8") as f:
        for row in csv.DictReader(f):
            rows.append(row)
    if not rows:
        raise ValueError("Magazine training CSV boş.")

    total_deliveries = len(rows)
    total_licks      = int(rows[-1].get("cumulative_licks", 0) or 0)
    avg_licks        = total_licks / total_deliveries if total_deliveries else 0
    ...
    c = ws1.cell(row=1, column=1, value=f"MAGAZINE TRAINING RAPORU — {animal_id}")
    c.fill = fill(C["accent"]); c.font = Font(color="FFFFFF", bold=True, size=16, name="Segoe UI")
    c.alignment = center; ws1.row_dimensions[1].height = 36
    ...
    summary_headers = ["Toplam Teslimat", "Toplam Lick", "Ort. Lick / Teslimat", "Süre (dk)"]
    summary_values  = [
        (str(total_deliveries), C["text"]),
    ...
    # ── Sayfa 2: Teslimat Detayları ────────────────────────────────────────────
    ws2 = wb.create_sheet("Teslimat Detayları")
    ws2.sheet_view.showGridLines = False
    ...
    for rx, r in enumerate(rows, 2):
        lick_this = int(r.get("lick_count_this_interval", 0) or 0)
        lick_fg   = C["green_fg"] if lick_this > 0 else C["red_fg"]
        write(ws2, rx, 1, int(r.get("delivery_num", 0) or 0),      bg=C["surface"], fg=C["text"])
